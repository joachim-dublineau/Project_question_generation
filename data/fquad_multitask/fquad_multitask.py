...
parser.add_argument("-pi", "--path_piaf_data", default="", type=str, help="path to json files.")
parser.add_argument("-pt", "--path_translated_data", default="", type=str, help="path to json files.")
...
if args.path_piaf_data != "":
    ...
    df_mix.to_csv("fr_train.csv")

else:
    df_train_fquad.to_csv("fquad_train.csv")

if args.path_translated_data != "":
    fquad_generator = _generate_examples(os.path.join(args.path_translated_data, "translated-train.json"), "highlight")
    sources, targets, tasks = [], [], []
    elem = next(fquad_generator, None)
    while elem is not None:
        sources.append(elem[1]['source_text'])
    ...
    df_mix = pd.concat([df_mix, df_train_train_trans])
    df_mix = df_mix.reset_index(drop=True)

    fquad_generator = _generate_examples(os.path.join(args.path_translated_data, "translated-valid.json"), "highlight")
    sources, targets, tasks = [], [], []
    elem = next(fquad_generator, None)
    while elem is not None:
        sources.append(elem[1]['source_text'])
    ...
    df_mix = pd.concat([df_mix, df_train_valid_trans])
    df_mix = df_mix.reset_index(drop=True)
    df_mix.to_csv("mix_train.csv")
